...
var_add(variable_s *var)
{
    ...
    entry.key = var->name;
    entry_rt = hash_search(&m_lexical_htab, entry, FIND);
    if (entry_rt) {
    ...
    entry.data = v;
    entry_rt = hash_search(&m_lexical_htab, entry, HASH_ENTER);
    if (!entry_rt) {
...
var_get(char *name)
{
    ...
    entry_rt = hash_search(&m_constant_htab, entry, FIND);
    if (entry_rt) {
